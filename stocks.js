...
search.addEventListener('submit',(e)=>{
  ...
  const data= fetch(`https://cloud.iexapis.com/stable/stock/${stock}/quote?token=${APIKey}`)
...
function fetchPrices(stock) {
  const prices=fetch(`https://cloud.iexapis.com/stable/stock/${stock}/chart?token=${APIKey}`)
      ...
      options: {
        ...
        scales: {
          x: {
            ...
            time: {
              unit: 'day', // Display unit as 'day' for better spacing
              displayFormats: {
                day: 'MMM DD', // Format for displaying date labels (e.g., "Jul 26")
              },
            },
            title: {
              display: true,
